...
def pairwise_distance_loss(true_coords, pred_coords, p=2):
    ...
    # Convert to float32 for pdist since it doesn't support Half precision on CUDA
    true_coords_f32 = true_coords.float()
    pred_coords_f32 = pred_coords.float()
    
    true_distances = torch.pdist(true_coords_f32, p=p)
    pred_distances = torch.pdist(pred_coords_f32, p=p)
    loss = F.mse_loss(pred_distances, true_distances)
    return loss
  
# 32 latent dimension, same edge_index for all 
...
def main():
    ...
    train_loader = DataLoader(
        ...
        batch_size=BATCH_SIZE,
        shuffle=True,
        num_workers=8,  # Disable multiprocessing to avoid macOS issues
        pin_memory=True  # Disable pin_memory when num_workers=0
    )
    ...
    #edge_index = train_data_list[0].edge_index
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if batch_idx % 50 == 0:
                ...
                print(f"  Debug - first few sigmas: {mu[0, 0, :3]}, log_var: {log_var[0, 0, :3]}")
        ...
        if epoch % 5 == 0:
            checkpoint_path = f'checkpoints_new/vae_model_pairwise_epoch{epoch}_small_model_with_changes.pth'
    ...
    final_model_path = 'checkpoints_new/vae_model_pairwise_final_small_model_with_changes.pth'
    # Prepare checkpoint data
    checkpoint_data = {
        'epoch': EPOCHS,
        'model_state_dict': model.state_dict(),
